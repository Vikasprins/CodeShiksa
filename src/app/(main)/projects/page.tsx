...
function ProjectCard({ project }: { project: typeof projects[number] }) {
  return (
    <CardContainer className="inter-var">
      <CardBody className="bg-gray-50 group/card dark:bg-black border border-black/[0.1] dark:border-white/[0.2] w-auto sm:w-[22rem] rounded-xl p-6 transition hover:shadow-2xl dark:hover:shadow-emerald-500/[0.1]">
          ...
          <div className="rounded-xl overflow-hidden">
            <CardItem translateZ="100">
              <Image
                ...
                width={500}
                height={300}
                className="h-70 w-full object-cover rounded-xl group-hover/card:shadow-xl transition"
              />
            </CardItem>
            <div className="p-4">
              <CardItem translateZ="50" className="space-y-2 text-neutral-600 dark:text-white">
        ...
        <div className="flex gap-3 flex-wrap">
          {categories.map((cat) => (
            <button
              key={cat}
              onClick={() => setSelectedCategory(cat)}
              className={`px-3 py-1 rounded-full text-sm border hover:bg-white hover:text-black transition ${
                selectedCategory === cat
                  ? "bg-[#045e52] text-white"
                  : "bg-white text-black border-[#045e52]"
              }`}
            >
              {cat}
            </button>
