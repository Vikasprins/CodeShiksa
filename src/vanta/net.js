...
class Effect extends VantaBase {
  static initClass() {
    this.prototype.defaultOptions = {
      color: 0xff3f81,    // dot color
      lineColor: 0xff3fff, // line color
      backgroundColor: 0x23153c,
      points: 10,
      maxDistance: 20,
      spacing: 15,
  ...
  genPoint(x, y, z) {
    ...
    sphere.oz = z
    sphere.position.set(x, y, z)
    sphere.r = rn(-2, 2) // rotation speed
  ...
  onUpdate() {
    ...
    const lineColor = new THREE.Color(this.options.lineColor)
    ...
    this.points.forEach((p) => {
      if (p.r !== 0) {
        const ang = Math.atan2(p.position.z, p.position.x) + 0.0005 * p.r
